...
Sheet format:
  ...
  Column B: Status dropdown — "Write Now" | "pause" | "DONE" | "Images o/s" | "Review" | "Publish"
  ...
  Column H: Review count (incremented each time a row is flagged "Review"; retained on DONE)
...
def read_pending(range_str: Optional[str] = None) -> list[dict]:
    ...
    for i, row in enumerate(rows):
        ...
        if status.lower() not in (QUEUE_VALUE.lower(), IMAGES_PENDING_VALUE.lower(), PUBLISH_VALUE.lower()):
            continue
...
def update_review_count(row_number: int, count: int) -> None:
    """Write review count to Column H of the given row."""
    service = get_service()
    sheet_id = get_sheet_id()
    service.spreadsheets().values().update(
        spreadsheetId=sheet_id,
