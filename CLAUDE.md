...
**Auto-deploy:** `.github/workflows/deploy-plugin.yml` deploys `wordpress/seomachine.php` to all 5 sites automatically on every push to `main` that touches that file. Three parallel jobs: GTM/GTB (`u2168-sqqieazmgeuw@ukm1.siteground.biz`), SDY (`u2732-2mxetksmslhk@gukm1055.siteground.biz`), and TMG/TMB (`u3520-kztrwuly6pid@uk1001.siteground.eu`). Uses `SITEGROUND_SSH_KEY` GitHub Actions secret (private key at `~/.ssh/seomachine_deploy`).
