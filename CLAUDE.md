...
**Auto-deploy:** `.github/workflows/deploy-plugin.yml` deploys `wordpress/seomachine.php`, `wordpress/seomachine-hub.css`, and `wordpress/seomachine-hub-v2.css` to all 5 sites automatically on every push to `main` that touches any of those files. Three parallel jobs: GTM/GTB (`u2168-sqqieazmgeuw@ukm1.siteground.biz`), SDY + staging2 (`u2732-2mxetksmslhk@gukm1055.siteground.biz`), and TMG/TMB (`u3520-kztrwuly6pid@uk1001.siteground.eu`). The SDY job deploys to both `serendipitymassage.co.uk` and `staging2.serendipitymassage.co.uk`. Uses `SITEGROUND_SSH_KEY` GitHub Actions secret (private key at `~/.ssh/seomachine_deploy`). **CSS cache-busting note:** SG Optimizer strips `?ver=` query strings from static asset URLs, making `wp_enqueue_style` version bumps ineffective for cache-busting. When CSS changes need to reach users immediately, rename the file (e.g. `seomachine-hub.css` → `seomachine-hub-v2.css`) and update both the PHP enqueue reference and the deploy workflow. Current active CSS file: `seomachine-hub-v2.css` (enqueued in `seomachine.php`, v3.4.5).
...
**SiteGround cache auto-purge** — `_purge_sg_cache()` in `WordPressPublisher` runs `sg_cachepress_purge_everything()` via WP-CLI SSH at the end of every successful `publish_html_content()` call. Requires `wp_path` in the client's `ssh` block. Currently active for GTM, GTB, SDY, and TMG. Silent no-op for clients without `ssh_config` or `wp_path`. Console output: `→ Cache: purged`. This ensures cross-site hub shortcodes on secondary blog sites (GTB, TMB) see fresh content immediately after publish — SiteGround otherwise serves cached empty REST responses to server-to-server PHP requests.
...
**SEO head output** (v3.4.5) — `seomachine.php` now fully owns all SEO head tags on singular pages; no SEO plugin (Yoast, Rank Math, etc.) required or expected. Outputs at `wp_head` priority 1: `<link rel="canonical">`, `<meta name="description">`, OG tags, Twitter Card tags. Also suppresses duplicates from WordPress core (`rel_canonical` at priority 10) and Hello Elementor theme (`hello_elementor_add_description_meta_tag` at priority 10). `add_filter('wp_robots', 'wp_robots_max_image_preview_large')` added at plugin level for large image previews in Google search.
