...
- **Competitor domain extraction:** Extract all `https?://[domain]` URLs from the markdown using the same regex pattern as `research_competitors.py`. Dedupe to root domains only. If fewer than 2 domains are found, skip Pass 1 with a warning.
- API: DataForSEO Backlinks API — use the **`/v3/backlinks/backlinks/live`** endpoint (synchronous; returns results immediately like all other existing DataForSEO methods). Note: higher credit cost per call than async endpoints — run against top 3–5 competitors only (not the full list).
- New method: `get_backlinks(domain)` added to `dataforseo.py`, following the existing `_post(endpoint, data)` pattern.
...
- Post-processing: filter results against a blocklist of social/directory domains (twitter.com, facebook.com, instagram.com, yelp.com, tripadvisor.com, yell.com, etc.). One mechanism only — no duplicate filtering in both the query string and post-processing.
...
@dataclass
class Opportunity:
    url: str            # normalised (trailing slash stripped, utm params removed, www normalised)
    title: str
    type: str           # competitor-backlink | resource-page | brand-mention
    domain_authority: int
    monthly_traffic: int | None   # None if DataForSEO doesn't return it
    referring_domains: int
    outreach_angle: str
    score: float        # computed
```

**URL normalisation** (applied before duplicate detection and Sheet writes): strip trailing slashes, strip `?utm_*` and other tracking parameters, normalise `www.` prefix to bare domain. Applied consistently across all three passes so duplicate detection works reliably on re-runs.
...
traffic_score: normalised 0–1 across all results in the current batch
               None values treated as 0 before normalisation
```

Normalisation is computed across the full merged result set before any `--type` or `--limit` filtering is applied, so scores are consistent regardless of which flags are used. Results written to Sheet ordered by score descending (highest-priority at top).
...
**Sheet initialisation:** On first run, the script checks whether the Sheet is empty (no rows). If empty, it writes the header row (columns A–L with field names) before appending data. This prevents duplicate detection from treating header text as a URL on subsequent runs.
